...
<body data-source="{source_name}">
    ...
    def _generate_job_card(self, job: Dict[str, Any], index: int) -> str:
        ...
        if job.get("company_rating"):
            ...
            glassdoor_badges.append(f'<span class="rating-badge" title="Glassdoor Company Rating">⭐ {rating:.1f}</span>')

        # Easy Apply badge
        if job.get("easy_apply"):
            glassdoor_badges.append('<span class="easy-apply-badge" title="Quick application process">✓ Easy Apply</span>')
        ...
        if job.get("salary_source") == "ESTIMATED":
            glassdoor_badges.append('<span class="salary-source-badge" title="Salary estimated by Glassdoor">💰 Est. Salary</span>')
            ...
            <div class="job-header">
                <div class="job-title-section">
                    ...
                    <div class="job-meta">
                        ...
                        {f'<span class="separator">•</span><span class="remote-badge">Remote</span>' if remote else ''}
                        {f'<span class="separator">•</span><span class="job-type">{job_type}</span>' if job_type else ''}
                    </div>
                    {salary_html}
        ...
        if company_rating:
            glassdoor_parts.append(f"""
                <div class="detail-row">
                    ...
                    <span class="detail-value">⭐ {company_rating:.1f}/5.0 (Glassdoor)</span>
                </div>
            """)
        ...
        if easy_apply:
            glassdoor_parts.append(f"""
                <div class="detail-row">
                    ...
                    <span class="detail-value">✓ Easy Apply (Quick application)</span>
                </div>
            """)
        ...
        if is_sponsored:
            ...
            glassdoor_parts.append(f"""
                <div class="detail-row">
                    ...
                    <span class="detail-value">⚠️ {sponsorship_level} (Promoted posting)</span>
                </div>
            """)
    ...
    def _format_analysis(self, job: Dict[str, Any]) -> str:
        ...
        gaps_html = "".join([f"<li class='gap-item'>⚠️ {g}</li>" for g in gaps])
        red_flags_html = "".join([f"<li class='red-flag-item'>🚩 {r}</li>" for r in red_flags]) if red_flags else ""
    ...
    def _format_recommendations(self, job: Dict[str, Any]) -> str:
        ...
        cover_letter_html = "".join([f"<li>💬 {c}</li>" for c in cover_letter])
    ...
    def _get_css(self) -> str:
        ...
        try:
            ...
            print(f"⚠️  Error reading CSS file: {e}")
